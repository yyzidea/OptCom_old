...
namespace Common{
	...
	{
		char idstr[17] = {0};
		_snprintf_s(idstr, sizeof(idstr), "COM%-13d", _i);
		std::stringstream ss;
		ss << idstr << "\t\t" << _s;
...
int GetPortNum(WORD vid, WORD pid, char* ser, BYTE ifc)
{
	...
	if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\", 0,
		KEY_READ, &tmpKey))
	{
		if (ERROR_SUCCESS == RegOpenKey(tmpKey, L"Enum\\USB\\", &tmpSubKey))
		{
				...
				else
				{
					...
					//If the portkey string is in the registry, then get the parent ID to start looking at
						...
						if (ERROR_SUCCESS == RegOpenKeyEx(tmpSubKey, portKeyString, 0, KEY_READ,
							&tmpPortKey))
						{
							if (ERROR_SUCCESS == RegQueryValueEx(tmpPortKey, L"ParentIdPrefix", NULL,
